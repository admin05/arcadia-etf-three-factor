...
如果 NAS 已经有 Chromium/Chrome，也可以指定。注意这里必须是真实存在的可执行文件路径：
...
如果日志里出现 `executable doesn't exist at /usr/bin/chromium`，说明 `CHROME_BIN` 指向了不存在的路径。可以先取消这个变量，让脚本自动查找 Playwright 自带浏览器和常见系统路径：
